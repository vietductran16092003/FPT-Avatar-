...
export class AppStore {
  ...
  /* ---- persistence bridge ----
     load() must fully REPLACE in-memory state with the persisted snapshot,
     not merge into it — this instance's constructor defaults only exist to
     seed the very first visit (no `raw` yet). A previous version guarded
     campaigns on `data.campaigns.length`, so deleting every campaign down
     to an empty array (a valid, saved state) was silently discarded and
     every other tab/page fell back to the hard-coded default campaigns —
     "deleted" campaigns kept reappearing on the public site. downloadCounts
     had the same class of bug via Object.assign, which merges keys in but
     never removes ones a delete had cleared. */
  load() {
    try {
      ...
      if (data.downloadCounts && typeof data.downloadCounts === 'object') this.downloadCounts = data.downloadCounts;
  ...
  saveCampaign(draft, editingSlug) {
    ...
    if (!isNew) {
      ...
      // Slug is editable on an existing campaign; downloadCounts is keyed by
      // slug, so a rename must carry its history forward instead of losing it.
      if (draft.slug && draft.slug !== editingSlug && editingSlug in this.downloadCounts) {
        this.downloadCounts[draft.slug] = this.downloadCounts[editingSlug];
        delete this.downloadCounts[editingSlug];
      }
    } else {
      this.campaigns.push({
  ...
  saveTemplate(campaignSlug, draft, editingTemplateId) {
    ...
    const newTpl = {
      ...
      name: draft.name, frameImageUrl: draft.frameImageUrl || null, photoArea: draft.photoArea, components: draft.components,
    };
